...
\title{Konfiguraèní management microservice}
...
I ve svìtì poèítaèù se vzhlíží k urèitým módním smìrùm, které se s neurèitou pravidelností mìní. Se vzrùstajícím výkonem hardware komponent IT svìta je evoluce v nehmatatelné oblasti naprosto nezbytná. Jak tyto zmìny dovedou s virtuálním svìtem zahýbat nám už ukázala napøíklad cloud technologie. Kromì konkrétních technologických a nebo hmatatelných zmìn se ale vyvíjí i metodické procesy vývoje aplikací. Stejnì jako se do IT pravìku dostal waterfall pøístup a nahradil ho pøístup agilní, jsou monolitické aplikace nahrazovány microservice architekturou. Touto architekturou se chci ve svojí práci zabývat.

Bohužel zatím neexistuje jednotný pøístup k vývoji aplikace, který by byl sám o sobì bez slabých míst. V tomto textu chci popsat problematiku jednoho z tìchto míst, kterým je konfiguraèní management. Proè se vlastnì jedná o slabé místo? Rád bych rozebral pøístup s jakým pøistupovat ke konfiguraènímu managementu v cloud native prostøedí. Nedílnou souèástí jsou konfiguraèní parametry a proto je nutné vysvìtlit jak s nimi zacházet. Jako vlastník aplikace budeme chtít omezit pøístup veøejnosti, vèetnì vývojáøù, k nìkterým parametrùm. Pokusím se nastínit práci s tìmito tajnými informacemi.

Konfiguraèní management byla v minulosti zodpovìdnost systémových adimistrátorù, kteøí k ní pøistupovali manuálnì. S nástupem DevOps se však toto zmìnilo. Velké množství manuální práce znamená mnoho pøíležitostí zpùsobit chybu. Lidské chybì se dá zabránit automatizací procesu, a proto už tato disciplína není praktikována manuálnì.
...
V závìru práce bych rád popsal k èemu slouží nástroje které danou problematiku výraznì ulehèují a nìkteré z nich porovnal.


1) Popsat best practices konfiguraèního managementu v cloud native prostøedí a jak používat konfiguraèní parametry v aplikacích.
2) Popsat jak obecnì pracovat se secrets, jak je ukládat a jaké jsou možnosti jejich šifrování.
3) Vytvoøit prototyp aplikace, ve které bude konfiguraèní management použit.
4) Nastínit problém nìkolika prostøedí, zdokumentovat postup nasazování a jak lze uplatnit management pøi migraci mezi prostøedími.
5) Zhodnotit výsledek práce, porovnat možné užití nástrojù pro konfiguraèní management.

Proè jsem si tuhle práci vybral?


Tento námìt práce jsem si vybral proto, jelikož se už dlouhou dobu pohybuji v korporátním svìtì, kde je komplexní sturktura nedílnou souèástí. Nezøídka kdy se stává že je ... (dopsat) ... A zde pøíchází na øadu konfigraèní management. Kolik dovede ušetøit vlastnì práce a je pak dodávání softwaru ménì problémové? 

Cíl práce.

Lépe rozepsat osnovu než tak jak je teï
\end{introduction}
